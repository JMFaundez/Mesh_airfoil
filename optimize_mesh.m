function optimize_mesh(m)
  ...
  % Parameters to tune
  nelx = [200, 165, 165, 200, 200];
  nely = [40, 40, 40, 34, 34];
  rexp = [0.55, 0.55, 0.55, 0.55, 0.35];
  xprcutup = [.5, 0.4, 0.4, 0.4, 0.4];
  xprcutlw = [.04, .04, .04, .04, .04];
  lin = [.3, .3, .3, .06, 0.06];
  xinup = [-.05, -.05, -.05, -.01, -.01];
  xinlw = [-.05, -.05, -.05, -.01, -.01];
  doutup = [.1, .1 , .1, 15*dis_th, 20*dis_th];
  doutlw = [.08, .08, .08, 10*dis_th, 15*dis_th ];
  
  span = [0.03, 0.03, 0.03, 0.02, 0.02];
  nelz = [18, 18, 18, 12, 12];
